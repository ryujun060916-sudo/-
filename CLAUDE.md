...
**2026年9月、クライアント支給のデザインファイル一式に合わせてサイト全体の配色をブラウン系からネイビーブルー系に変更済み**（`docs/DESIGN_TEMPLATE.md` に記載のブラウン系色見本は旧配色のサンプルであり、現在は使用していない。旧配色からの単純な置換なので、レイアウト・タイポグラフィ・コンポーネント構造に変更はない）。

### カラーパレット（`site/css/style.css` 実値）
| 役割 | カラーコード | 用途 |
| --- | --- | --- |
| Primary（ブランドネイビーブルー） | `#2F5D8C` | ボタン・リンク・アクセント |
| Primary Dark（ホバー／見出しルール） | `#1F3A5F` | ホバー時・院名・区切り線 |
...
| Accent Sub（アイコン等） | `#8AAECD`, `#17293D`, `#46535F` | 装飾・濃淡バリエーション |
| Accent Blue（英字ロゴ・重要CTA） | `#243EB3` | `.site-header__en`、予約ボタン等（変更前から同色） |
| Footer 濃紺（2層） | `#20456B`（予約／アクセス帯） / `#16283D`（サイトマップ帯） | `.reserve` / `.site-footer` 背景 |
