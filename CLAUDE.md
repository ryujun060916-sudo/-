...
**2026年9月、クライアント支給のデザインファイル一式に合わせてサイト全体の配色をブラウン系からブルー系に変更済み**（`docs/DESIGN_TEMPLATE.md` に記載のブラウン系色見本は旧配色のサンプルであり、現在は使用していない。旧配色からの単純な置換なので、レイアウト・タイポグラフィ・コンポーネント構造に変更はない）。同月中にクライアントの参考画像（ピクセルサンプリングで実測）に合わせて、当初のダークネイビー案から、より明るく彩度の高いミディアムブルーへ再調整済み。

### カラーパレット（`site/css/style.css` 実値）
| 役割 | カラーコード | 用途 |
| --- | --- | --- |
| Primary（ブランドブルー） | `#0A6FA8` | ボタン・リンク・アクセント・ステップ番号 |
| Primary Dark（ホバー／見出しルール／フッター2層とも） | `#075A8A` | ホバー時・院名・区切り線・`.reserve`/`.site-footer`背景・ヒーロー暗背景 |
| Text Main | `#1A1A1A` | 本文・見出し |
| Text Muted | `#5E6B7A` | 補助テキスト・ラベル |
| Border / 薄ブルーグレー | `#D3DFEA` | 罫線・カード境界 |
| Background 淡色 | `#F2F6FA` / `#E9EFF6` 等 | セクション背景の淡いブルー系 |
| Accent Sub（アイコン等） | `#8AAECD`, `#46535F` | 装飾・濃淡バリエーション |
| Accent Blue（英字ロゴ・重要CTA） | `#243EB3` | `.site-header__en`、予約ボタン等（変更前から同色） |
| Alert（要確認時のみ） | `#B4453A` | エラー・警告表示（変更前から同色） |
| White | `#fff` | 背景・ボタン内テキスト |

**変更していない背景色**（暖色系のまま据え置き。デザインファイルでも意図的に維持されているニュートラル寄りのトーン）: `#F1F3F7` / `#F7F8FA`（ステップ・工程の交互背景、タグラベル背景）、`#F3EFEC` / `#F4EFEA`（メリットカード・比較カードの背景）。

汎用デザインテンプレートのようなCSS変数（`--color-primary` 等）は定義されておらず、**カラーコードを直接指定**するスタイル。新しいスタイルを追加する場合も既存の指定方法（直接hex値）に合わせる。トークンを増やす場合は、まずこのパレット内の値の再利用を検討し、新色が必要な場合はネイビーブルー系の色相に統一すること（旧ブラウン系・テンプレートのインディゴ／エメラルドは使用しない）。
...
### コンポーネント
- **ボタン**: `border-radius: 6px`（一部 `8px`）、ブランドネイビーブルー背景＋白文字が基本。
